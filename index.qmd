---
title: "Quantified Palaeoecology - Rates of Change and Ordination"
author: "Ian Matthews"
format: 
  html:
    toc: true
...
Exploration of data is often required in datasets with lots of variables, observations, or both. The principal way I undertake this is multivariate exploratory data analysis. I use this approach to understand the major changes within a dataset but also for dimension reduction to help build better models of change. The rescaling of data along the line of maximum variation captures the most variance while reducing the total number of variables to examine. This approach can often also help us to better understand the links between observations and variables by plotting ordination scatter plots.In this session, I will first cover data plotting and determining rates of change (RoC) in samples, then I will cover four approaches to multivariate analysis. The first three are classical ordination approach using Principal components analysis (PCA), Correspondence analysis (CA), and its detrended version (DCA). Last, I will highlight the potential of direct ordination to explain how much variability can be attributed to specific climatic forcing of our species data. The first parts will use pollen data while the last will utilise the Norwegian modern chironomid dataset.
...
If you are using Quarto or Rmarkdown you may wish to produce a better looking table of your data. This is particularly true if you wish to present material on slides. I use the gt package for this.

```{r warning=FALSE}
poll %>% 
  round(digits = 2) %>% 
  gt()
 # this rounds our data.frame to two decimal places and then produces a table of the data.
```
...
# Make new spec file using poll where mx is greater than 3 %  to remove rare taxa.
spec <- poll2[, mx >3]
...
This suggests that four or five different groups are viable in our data. We can add this information to our diagram and also tidy it up a little.
...
While we have reduced and simplified our plots, we haven't really explored our data. The zonation picks out periods of change in the data but doesn't allow us to understand this in a clear dataset. We need to report rates of change, and then we need to go beyond this and try to understand both the relationships between taxa (variables) and depths (observations). We can undertake ordination to achieve this. We have three main options for this, Correspondence analysis (CA), Detrended correspondence analysis (DCA - decorana), and Principal Components Analysis (PCA). There are a whole other suite of techniques we could explore, but these go beyond our needs in these data.

## Rate of Change (RoC)

We will constrain the rate of change in the Abernethy data using a dissimilarity measure. This looks to see how far samples vary from each other. Ideally this is calculated against age so that change over time can be identified, as we do not have a sufficient chronology for this we can plot against depth as a proxy for time.

First we will calculate dissimilarity using a Squared Chord Distance using the Hellinger method.

```{r}
distance_matrix <- as.matrix(vegdist(poll2, method = "hellinger", upper = TRUE, diag = FALSE))
distances <- diag(distance_matrix[-1, -ncol(distance_matrix)]) ## this extracts the most important information between sample differences.
```

Now we will need to determine the dissimilarity against depth.

```{r}
# Compute depth intervals (differences in depth)
depth_intervals <- diff(poll$Depth)

# Compute rate of change
rate_of_change <- distances / depth_intervals

# Combine into a dataframe
roc_df <- data.frame(
  depth = Depth[-1],  # Associate rate of change with the second sample in each pair
  RoC = rate_of_change
)
...
Last we will print this against depth to see where the biggest changes occur.

```{r}
ggplot(roc_df, aes(x = depth, y = RoC)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(title = "Rate of Change in Fossil Pollen Data",
       x = "Depth (cm)",
       y = "Rate of Change")
```

This suggests three locations with the greatest RoC, these closely agree with the zone boundaries. The greatest zones of change occurs at the base of the record and we might find that adding a smooth line through the data will show longer term trends.
...
This generates interesting results. The smooth shows highest RoC at the start of the record which declinestoward 480 cm. This trend is interrupted by higher values between 480 - 460 cm. This suggests that the onset of Windermere interstadial conditions and the onset of the Holocene lead to the greatest changes in vegetation assemblages.

## Linear techniques -

### PCA

Ordination via PCA rescales our data utilising the line of maximum variation. It attempts to capture as much of the total variability found within a sample as possible. We will undertake this for the Abernethy data. To ensure the data are comparable we will first transform them through scaling and centering the data first. We will first look at the output and produce a scree plot to see how much variation can be captured by each axis.
...
Axis 1 captures 33.7% of the total variation, while axis 2 captures 17.9%. Both of these capture c. 51.58% of the toal variation in the data. This is good to know but it doesn't tell us which variables are most important. This requires a second plot including this information.
...
This biplot shows the position of our variables. Those most closely aligned to axis 1 influence it most. The colours in this plot also show the total contribution (importance). In this case *Betula*, *Rumex* and *Artemisia* are important for axis one. as the arrows point in different directions it means that while *Betula* levels are high, *Artemisia* and *Rumex* are low and vice versa. Poaceae is important for axis 2.
...
This plot shows the samples (observations) against the variables and show which taxa are present in those samples e.g. samples 1-11 have *Betula*, *Calluna*, *Corylus*, *Juniperus* in them. When all these plots are combined we can see axis one is driven by stable woody vegetation at low values \<0 and unstable herbaceous vegetation as higher values \>0

However, the PCA has the issue surrounding monotone responses and doesn't relfect unimodal change.

## Unimodal techniques

Technically these should be better for ecological data we will check this for our data and see whether it provides a better assessment of change.

### Correspondence analysis (CA)

This technique might better reflect the changes but can be strongly influenced by the arch effect. Can can see this in the plot below.
...
This breaks our data into segments using linear regression and then 'flattens out' the data. It has some advantages in that the axis lengths become measures of standard deviation in the data. The code below gives different information to a PCA and requires a bit more transformation to make sense of it.
...
We see axis lengths \> 1.5-2 which means significnat change in our assemblage, remember that axis length units are now standard deviations. The length means this is likely to be the appropriate technique to use. We can convert this data to show how much variation is being captured.
...
This give each axis against the total inertia (the sum of all the Eigenvalues). From this we can calculate a percentage figure.
...
An alternative is to use ggplot2, but this requires some data manipulation. Note also the above plot is scaled to maximise the spacing between both samples and species, the output of the DCA does not automatically undertake that scaling.
...
This is much clearer, but we have lost some information on plotting of the taxa. It would be great if we could use colour in order to try and improve our understanding of what is going on here. We can do this by using the pollen zones already assigned and adding them to our samples data.frame. We do this below by adding a new sample group vector to our data set and using this to determine colour and symbol.
...
The table shows us that the groups have been successfully allocated, now we can plot this.
...
This shows us that zone 1 and three have the most positive position and therefore most open ground taxa with zone four having the most tree and woddy shrub cover. Zone 2 is more variable.
...
There is no 'correct' choice, you need to think about what it means for your own data. However, here `rclr` changes the data the most. You should now be able to use linear approaches to your data (RDA and PCA). The next few lines of code will perform PCA plots on the transformed data, with Hellinger first and then rclr. Look at these and compare them to the untransformed data you produced above.

```{r}
pcascores_trans <- prcomp(spec_trans, scale. = TRUE, center = TRUE)
summary(pcascores_trans) ## hellinger transform applied
...
The Hellinger transform means 41.98% of all variation is captured in axis 1 with another 17.68% in axis 2. The most important variables are the same as in the untransformed data.

```{r}
pcascores_trans2 <- prcomp(spec_trans2, scale. = TRUE, center = TRUE)
summary(pcascores_trans2) # rclr method
...
The rclr method captures 27.67% variation in axis 1 and 17.51% in axis 2. The approach also reduces the importnance of tree cover relative to Poaceae, *Empetrum* and Cyperaceae. Applying your ecological knowledge is important here to know whether this is appropriate.
...
The PCA(rclr) produces a result which looks unlike any of the other approaches and you will need to think about whether this is helpful. Conversly the Hellinger transformed PCA captures the most variation of any approach in axis one (nearly 42%), it seems an interesting combination of both PCA and DCA results and this might add credibility.

## Direct Ordination

We will use a modern dataset for this direct ordination where we are trying to determine which environmental variables influence species abundances. We will use a Norwegian chironomidae dataset collected from lakes from Svalbard down to Southern Norway. The samples collected were the larval stage from within the lake. At each location modern environmental data were collected from those locations e.g. water quality, temperature etc... The objective is to try and to determine how species assemblages change with differing environmental conditions. For this we need two tidy data sheets. One ordered by site containing the species data and a second ordered by site including the environmental data. This work was originally published by @brooks2000. Ideally, the last stage would be to apply these trends to a palaeodataset, but this requires additional stages and checks on the data. We can perform this another time if you wish to know more about this.
...
```{r}
mod0 <- cca(N_chiros_trans ~ 1, NORENV_t) # unconstrained
mod0
mod1 <- cca(N_chiros_trans ~ ., NORENV_t) # constrained with environmental variables.
mod1
```

```{r}
mod <- step(mod0, scope = formula(mod1), test = "perm") ## from all variables to entirely unconstrained.
```
